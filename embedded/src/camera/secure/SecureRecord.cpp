...
camera::base::Expected<std::vector<uint8_t>, std::string> SecureRecord::seal(
    const uint8_t* plaintext, size_t length) {
    if (length > kMaxPlaintext || length > std::numeric_limits<int>::max())
        return camera::base::makeUnexpected(std::string("secure record too large"));

    EVP_CIPHER_CTX* ctx = EVP_CIPHER_CTX_new();
    if (ctx == nullptr)
        return camera::base::makeUnexpected(openssl_error("EVP_CIPHER_CTX_new"));
    ...
    if (EVP_EncryptInit_ex(ctx, EVP_chacha20_poly1305(), nullptr, nullptr,
        ...
        return camera::base::makeUnexpected(openssl_error("ChaCha20-Poly1305 encrypt"));
    int final = 0;
    if (EVP_EncryptFinal_ex(ctx, out.data() + written, &final) != 1 ||
        EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_GET_TAG, kTagLength,
                            out.data() + length) != 1)
        return camera::base::makeUnexpected(openssl_error("ChaCha20-Poly1305 finalize"));
    return out;
}

camera::base::Expected<std::vector<uint8_t>, std::string> SecureRecord::open(
    const uint8_t* ciphertext_and_tag, size_t length) {
    if (length < kTagLength || length - kTagLength > kMaxPlaintext ||
        length > std::numeric_limits<int>::max())
        return camera::base::makeUnexpected(std::string("invalid secure record length"));
    ...
    if (EVP_DecryptInit_ex(ctx, EVP_chacha20_poly1305(), nullptr, nullptr,
        ...
        return camera::base::makeUnexpected(openssl_error("ChaCha20-Poly1305 decrypt"));
    int final = 0;
    if (EVP_DecryptFinal_ex(ctx, out.data() + written, &final) != 1)
        return camera::base::makeUnexpected(std::string("secure record authentication failed"));
    return out;
}
