...
// A discovery request is exactly a JSON object with "method": "discover";
    ...
    if (json_parser_load_from_data(parser, data, -1, nullptr)) {
        ...
        if (root != nullptr && JSON_NODE_HOLDS_OBJECT(root)) {
            ...
            const char* method = json_object_get_string_member_with_default(
                obj, "method", nullptr);
            ok = method != nullptr && g_strcmp0(method, "discover") == 0;
        }
    }
...
bool DiscoveryServer::start() {
    ...
    g_object_unref(addr);
    if (!ok) {
        g_printerr("discovery: bind 0.0.0.0:%d/udp failed: %s\n",
                   config_.discovery_port, err->message);
        g_error_free(err);
        return false;
    }
    ...
    g_message("discovery responder on 0.0.0.0:%d/udp", config_.discovery_port);
    return true;
}
