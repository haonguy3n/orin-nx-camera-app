...
folly::Expected<PkeyPtr, std::string> make_p256_key() {
    EVP_PKEY_CTX* raw = EVP_PKEY_CTX_new_id(EVP_PKEY_EC, nullptr);
    if (raw == nullptr)
        return folly::makeUnexpected(std::string("P-256 context creation failed"));
    ...
    if (EVP_PKEY_keygen_init(raw) <= 0 ||
        ...
        return folly::makeUnexpected(std::string("P-256 key generation failed"));
    return PkeyPtr(key, EVP_PKEY_free);
}

folly::Expected<std::array<uint8_t, 65>, std::string> public_point(EVP_PKEY* key) {
    ...
    if (EVP_PKEY_get_octet_string_param(key, OSSL_PKEY_PARAM_PUB_KEY,
        ...
        return folly::makeUnexpected(std::string("invalid P-256 public key"));
    return result;
}

folly::Expected<PkeyPtr, std::string> peer_key(
    const std::array<uint8_t, 65>& point) {
    if (point.front() != 0x04)
        return folly::makeUnexpected(std::string("invalid P-256 point"));
    EC_KEY* raw_ec = EC_KEY_new_by_curve_name(NID_X9_62_prime256v1);
    if (raw_ec == nullptr)
        return folly::makeUnexpected(std::string("P-256 peer context failed"));
    ...
    if (o2i_ECPublicKey(&raw_ec, &cursor, point.size()) == nullptr ||
        ...
        return folly::makeUnexpected(std::string("P-256 peer key creation failed"));
...
folly::Expected<std::array<uint8_t, 32>, std::string> shared_secret(
    EVP_PKEY* private_key, EVP_PKEY* peer) {
    EVP_PKEY_CTX* raw = EVP_PKEY_CTX_new(private_key, nullptr);
    if (raw == nullptr)
        return folly::makeUnexpected(std::string("ECDH context creation failed"));
    std::unique_ptr<EVP_PKEY_CTX, decltype(&EVP_PKEY_CTX_free)> context(
        raw, EVP_PKEY_CTX_free);
    ...
    if (EVP_PKEY_derive_init(raw) <= 0 || EVP_PKEY_derive_set_peer(raw, peer) <= 0 ||
        ...
        return folly::makeUnexpected(std::string("P-256 ECDH failed"));
    return secret;
}
...
folly::Expected<std::vector<uint8_t>, std::string> sign(EVP_PKEY* key,
    ...
    if (!context || EVP_DigestSignInit(context.get(), nullptr, EVP_sha256(), nullptr, key) != 1 ||
        ...
        return folly::makeUnexpected(std::string("handshake signature initialization failed"));
    size_t length = 0;
    if (EVP_DigestSignFinal(context.get(), nullptr, &length) != 1 || length > kMaxSignatureSize)
        return folly::makeUnexpected(std::string("handshake signature sizing failed"));
    std::vector<uint8_t> result(length);
    if (EVP_DigestSignFinal(context.get(), result.data(), &length) != 1)
        return folly::makeUnexpected(std::string("handshake signature failed"));
    result.resize(length);
    return result;
}

folly::Expected<bool, std::string> verify(EVP_PKEY* key,
                                          const std::vector<uint8_t>& data,
    ...
    if (!context || EVP_DigestVerifyInit(context.get(), nullptr, EVP_sha256(), nullptr, key) != 1 ||
        ...
        return folly::makeUnexpected(std::string("handshake verification initialization failed"));
    return EVP_DigestVerifyFinal(context.get(), signature.data(), signature.size()) == 1;
}

folly::Expected<X509Ptr, std::string> read_pem_certificate(const std::string& path) {
    std::unique_ptr<FILE, decltype(&fclose)> file(fopen(path.c_str(), "r"), fclose);
    if (!file)
        return folly::makeUnexpected(std::string("cannot read pinned certificate: ") + path);
    X509* cert = PEM_read_X509(file.get(), nullptr, nullptr, nullptr);
    if (cert == nullptr)
        return folly::makeUnexpected(std::string("invalid PEM certificate: ") + path);
...
folly::Expected<folly::Unit, std::string> verify_against_trust_anchor(
    ...
    if (!store)
        return folly::makeUnexpected(std::string("cannot allocate certificate store"));
    if (X509_STORE_load_locations(store.get(), path.c_str(), nullptr) != 1)
        return folly::makeUnexpected(std::string("cannot read trust anchor: ") + path);
    ...
    if (!ctx || X509_STORE_CTX_init(ctx.get(), store.get(), presented, nullptr) != 1)
        return folly::makeUnexpected(std::string("cannot initialise certificate verification"));
...
folly::Expected<PkeyPtr, std::string> read_pem_private_key(const std::string& path) {
    std::unique_ptr<FILE, decltype(&fclose)> file(fopen(path.c_str(), "r"), fclose);
    if (!file)
        return folly::makeUnexpected(std::string("cannot read private key: ") + path);
    EVP_PKEY* key = PEM_read_PrivateKey(file.get(), nullptr, nullptr, nullptr);
    if (key == nullptr)
        return folly::makeUnexpected(std::string("invalid private key: ") + path);
    return PkeyPtr(key, EVP_PKEY_free);
}

}  // namespace

folly::Expected<std::vector<uint8_t>, std::string> encode_client_hello(const ClientHello& hello) {
...
folly::Expected<ClientHello, std::string> decode_client_hello(const std::vector<uint8_t>& wire) {
    if (wire.size() != kClientHelloSize ||
        !std::equal(std::begin(kClientHelloMagic), std::end(kClientHelloMagic), wire.begin()))
        return folly::makeUnexpected(std::string("invalid secure USB ClientHello"));
    ClientHello hello;
...
folly::Expected<std::vector<uint8_t>, std::string> encode_server_hello(const ServerHello& hello) {
    if (hello.certificate_der.empty() || hello.certificate_der.size() > kMaxCertificateSize ||
        ...
        return folly::makeUnexpected(std::string("invalid secure USB ServerHello fields"));
...
folly::Expected<ServerHello, std::string> decode_server_hello(const std::vector<uint8_t>& wire) {
    constexpr size_t fixed = 4 + 32 + 65 + 2 + 2;
    if (wire.size() < fixed ||
        !std::equal(std::begin(kServerMagic), std::end(kServerMagic), wire.begin()))
        return folly::makeUnexpected(std::string("invalid secure USB ServerHello"));
    ...
    if (cert_length == 0 || cert_length > kMaxCertificateSize || signature_length == 0 ||
        ...
        return folly::makeUnexpected(std::string("invalid secure USB ServerHello length"));
    ServerHello hello;
    std::copy_n(wire.begin() + 4, hello.nonce.size(), hello.nonce.begin());
    std::copy_n(wire.begin() + 36, hello.public_key.size(), hello.public_key.begin());
...
folly::Expected<SessionKeys, std::string> ClientHandshake::complete(
    ...
    if (!presented || cursor != hello.value().certificate_der.data() + hello.value().certificate_der.size())
        return folly::makeUnexpected(std::string("invalid device certificate in handshake"));
    auto trusted = verify_against_trust_anchor(presented.get(), pinned_certificate);
    if (!trusted.hasValue())
        return folly::makeUnexpected(trusted.error());
    PkeyPtr signing_key(X509_get_pubkey(presented.get()), EVP_PKEY_free);
    if (!signing_key)
        return folly::makeUnexpected(std::string("device certificate has no public key"));
...
DeviceHandshake::respond(const std::vector<uint8_t>& client_wire,
    ...
    if (!certificate.hasValue() || !signing_key.hasValue() || !ephemeral.hasValue())
        return folly::makeUnexpected(!certificate.hasValue() ? certificate.error()
    ...
    if (cert_length <= 0 || static_cast<size_t>(cert_length) > kMaxCertificateSize)
        return folly::makeUnexpected(std::string("device certificate DER encoding failed"));
    ...
    if (!signature.hasValue())
        return folly::makeUnexpected(signature.error());
    server.signature_der = signature.value();
    auto wire = encode_server_hello(server);
    if (!wire.hasValue())
        return folly::makeUnexpected(wire.error());
    ...
    if (!keys.hasValue())
        return folly::makeUnexpected(keys.error());
